...
<html>
  <head>
    ...
    <script type="text/javascript">
       function toSub(){
            $.post("sys/updateCustomerTypeAction.action",$("#customertypeForm").serialize(),function(data){
...
<body>
    <div class="formbody">
    <form action="sys/updateCustomerTypeAction.action" method="post" id="customertypeForm" enctype="multipart/form-data">
    <ul class="forminfo">
        <li><label>类型名称</label><input name="customerType.c_type_name" type="text" class="dfinput" value="${customerType.c_type_name}"/><i>名称不能超过30个字符</i></li>
        <li><label>打折率</label><input name="customerType.c_discount" type="text" class="dfinput" value="${customerType.c_discount}"/><i>多个关键字用,隔开</i></li>
         <li style="width: 300px;height: 170px;"><label>头像</label>
            ...
            <li style="width: 350px;height: 170px;">
                <div id="localImag"><img id="preview" src="<%=path%>/upload/customertype/<s:property value="customerType.photo" default="guest.png"/> />"/></div>
            </li>
            <input type="hidden" name="role.role_id" value="${customerType.c_type_id}">
            <input type="hidden" id="old_role_name" value="${customerType.c_type_name}">
